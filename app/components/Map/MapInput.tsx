...
const Map = ({
  ...
  const [markerPosition, setMarkerPosition] = useState<[number, number]>([latitude, longitude]) // Default marker position
  const [address, setAddress] = useState('') // Address display

  const API_KEY_OPENCAGE = process.env.NEXT_PUBLIC_OPENCAGE_API_KEY;


  // Fetch address from coordinates
  const fetchAddress = async (lat: number, lng: number) => {
    const response = await fetch(
      `https://api.opencagedata.com/geocode/v1/json?q=${lat}+${lng}&key=${API_KEY_OPENCAGE}`
    )
    const data = await response.json()
    if (data.results && data.results.length > 0) {
      const formattedAddress = data.results[0].formatted
      setAddress(formattedAddress)
      onLocationChange({ latitude: lat, longitude: lng, address: formattedAddress })
    }
  }
  ...
  const MapClickHandler = () => {
    useMapEvents({
      click(e) {
        ...
        fetchAddress(lat, lng) // Fetch address after clicking
      }
    })
    return null
  }
  ...
  return (
    <div>
      ...
      >
        ...
        <MapClickHandler />
        <Marker
          position={markerPosition} // Update marker position dynamically
          icon={markerIcon}
        ></Marker>
      </MapContainer>
      <p className="mt-2 text-gray-700">Selected Address: {address || 'Click on the map to select a location'}</p>
    </div>
  )
}
